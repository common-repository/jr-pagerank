<?php
/*
Plugin Name: JR PageRank
Plugin URI: http://www.jakeruston.co.uk/2010/06/wordpress-plugin-jr-pagerank/
Description: Allows you to display the Google PageRank of each of your pages on your website!
Version: 1.0.4
Author: Jake Ruston
...
add_action('admin_menu', 'jr_PageRank_add_pages');

// action function for above hook
function jr_PageRank_add_pages() {
    add_options_page('JR PageRank', 'JR PageRank', 'administrator', 'jr_PageRank', 'jr_PageRank_options_page');
}
...
if (!function_exists("jr_PageRank_refresh")) {
function jr_PageRank_refresh() {
update_option("jr_submitted_PageRank", "0");
}
}

register_activation_hook(__FILE__,'PageRank_choice');

function PageRank_choice () {
if (get_option("jr_PageRank_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_PageRank";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_PageRank", "1");
wp_schedule_single_event(time()+172800, 'jr_PageRank_refresh'); 
} else {
...
update_option("jr_PageRank_links_choice", $content);
}
}

if (get_option("jr_PageRank_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_PageRank_link_personal", $content);
}
}

// jr_PageRank_options_page() displays the page content for the Test Options submenu
function jr_PageRank_options_page() {

    // variables for the field and option names
    $opt_name_1 = 'mt_PageRank_type';	
    $opt_name_5 = 'mt_PageRank_plugin_support';
	$opt_name_6 = 'mt_PageRank_title';
    $hidden_field_name = 'mt_PageRank_submit_hidden';
	$data_field_name_1 = 'mt_PageRank_type';
    $data_field_name_5 = 'mt_PageRank_plugin_support';
	$data_field_name_6 = 'mt_PageRank_title';
...
<?php
    ...
    echo "<h2>" . __( 'JR PageRank Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
<?php
    ...
    $change3 = get_option("mt_PageRank_plugin_support");
	$change4 = get_option("mt_PageRank_type");
...
<p><?php _e("Type of PageRank:", 'mt_trans_domain' ); ?> 
<input type="radio" name="<?php echo $data_field_name_1; ?>" value="0" <?php echo $change4; ?>>Show Homepage PageRank
<input type="radio" name="<?php echo $data_field_name_1; ?>" value="1" <?php echo $change41; ?>>Show Individual Pages PageRank
</p><hr />
...
function show_PageRank($args) {
extract($args);

  $widget_title = get_option("mt_PageRank_title"); 
  $option = get_option("mt_PageRank_option");
  
  if ($widget_title=="") {
  $widget_title="PageRank";
  }
...
?>
<img src="http://www.free-pagerank-checker.com/pr.php?url=<?php echo $siteurl; ?>"  border="0" alt="PageRank" />
<?php
}
}

function PageRank_footer_plugin_support() {
$supportplugin=get_option("mt_PageRank_plugin_support");
if ($supportplugin=="" || $supportplugin=="Yes") {
$linkper=utf8_decode(get_option('jr_PageRank_link_personal'));

if (get_option("jr_PageRank_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_PageRank_links_choice'));
...
update_option("jr_PageRank_links_choice", $new);
update_option("jr_PageRank_link_newcheck", "444");
}

if (get_option("jr_submitted_PageRank")=="0") {
$pname="jr_PageRank";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_PageRank", "1");
...
wp_schedule_single_event(time()+172800, 'jr_PageRank_refresh'); 
} else if (get_option("jr_submitted_PageRank")=="") {
$pname="jr_PageRank";
$url=get_bloginfo('url');
$current=get_option("jr_PageRank_links_choice");
...
wp_schedule_single_event(time()+172800, 'jr_PageRank_refresh'); 
  ...
  $pshow = "<p style='font-size:x-small'>PageRank Plugin created by ".$linkper." - ".stripslashes(get_option('jr_PageRank_links_choice'))."</p>";
  echo $pshow;
}
}

add_action("wp_footer", "PageRank_footer_plugin_support");

function init_PageRank_widget() {
register_sidebar_widget("JR PageRank", "show_PageRank");
}

add_action("plugins_loaded", "init_PageRank_widget");
